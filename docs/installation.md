...
| Account | A user-owned ChatGPT account that can sign in at `chatgpt.com`. Available models, reasoning levels, and web search depend on that account and ChatGPT's current product rules. A one-shot `ask` also works signed out, with the limits described under [Ask ChatGPT](../README.md#ask-chatgpt). |
